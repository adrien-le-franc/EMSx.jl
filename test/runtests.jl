...
const test_metadata2_path = joinpath(test_metadata_directory, "metadata2.csv")
...
@testset "EMSx.jl test set" begin
    ...
    @testset "Testing downloading data from Zenodo" begin
        ...
        @test isnothing(EMSx.initialize_data(test_data_directory, 
                                                ...
                                                compressed=true))
        @test !isfile(joinpath(test_data_directory, "8.csv.gz"))
        @test isfile(joinpath(test_data_test_directory, "8.csv.gz"))
        @test isfile(joinpath(test_data_train_directory, "8.csv.gz"))
        rm(joinpath(test_data_test_directory, "8.csv.gz"))
        rm(joinpath(test_data_train_directory, "8.csv.gz"))
    end
    @testset "EMS simulator's body" begin
        ...
        @testset "Parallel computing" begin
            ...
            average_costs = EMSx.average_cost_per_site(joinpath(test_data_save_directory, "score.jld2"))
            @test length(average_costs[!, :dummy]) == 3
